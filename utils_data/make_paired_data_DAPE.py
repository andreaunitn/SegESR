...
def realesrgan_degradation(batch,  args_degradation, use_usm=True, sf=4, resize_lq=True):
    ...
    # Need to resize images to desired sizes. Group [resize back + sinc filter] together
    # as one operation.
    # Consider two orders:
    #   1. [resize back + sinc filter] + JPEG compression
    #   2. JPEG compression + [resize back + sinc filter]
    # Empirically, find other combinations (sinc + JPEG + Resize) will introduce twisted lines.
